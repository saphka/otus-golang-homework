...
func TestRun(t *testing.T) {
	...
	t.Run("negative error count", func(t *testing.T) {
		...
		workersCount := 10
		maxErrorsCount := -1
		numRoutinesBefore := runtime.NumGoroutine()
		err := Run(tasks, workersCount, maxErrorsCount)
		...
		require.Eventuallyf(t, func() bool {
			numRoutinesAfter := runtime.NumGoroutine() - 1 // current routine
			return numRoutinesBefore == numRoutinesAfter
		}, 1*time.Second, 100*time.Millisecond, "not all goroutines stopped")
	})

	t.Run("zero error count - always error", func(t *testing.T) {
		tasksCount := 50
		tasks := make([]Task, 0, tasksCount)
...
func TestRunWorkers(t *testing.T) {
	...
	t.Run("zero worker count - sequential", func(t *testing.T) {
		tasksCount := 50
		tasks := make([]Task, 0, tasksCount)
